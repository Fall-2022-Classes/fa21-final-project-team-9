...
void AdsrCore::play_note(int freq, int dur) {
   int sus_tmp;
...
bool AdsrCore::play_note(int note, int oct, int dur) {
   int sus_tmp;
   int freq;
   bool done = false;
   if (!playing)
...
bool AdsrCore::play_note_p(Sound *sound_p) {
   return play_note(sound_p->note, sound_p->octave, sound_p->duration);
}
