...
public:
    ...
    *
    */
	int x = 0, y=  0, speedx = 0,speedy =0,centerx = 0, centery = 0;
	int size;
	int32_t ctrl;   // sprite memory size
	enum {
      BYPASS_REG = 0x2000,     /**< bypass control register */
   ...
   /**
       * move sprite by the x and y
       * @param x movement
       * @param y movement
       *
       * @note origin is the top-left corner of sprite
       */
  void moveXY(int xpos, int ypos, int exitScreen = 0);
  void moveDir();
  void stop();
  void get_start();
   ...
   void changeSprite(int32_t sprite);
...
public:
   ...
   void write(uint8_t x, uint8_t y, char ch[], int reverse = 0);
   int write (uint8_t x, uint8_t y, int num, int reverse = 0);
...
public:
   ...
   void drawBackground();
   void drawTombstone(int xpos, int ypos);
   void writeArray(int colors[], int x, int y, int size);
